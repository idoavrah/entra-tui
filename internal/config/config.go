...
func Load(args []string, getenv func(string) string, out io.Writer) (Config, error) {
	...
	var (
		...
		resource = fs.String("view", "users", "view to open on: users, groups, apps or sp")
	)
...
entra-tui only ever issues GET requests to Microsoft Graph, using your own
delegated permissions. It cannot modify your directory.
`
